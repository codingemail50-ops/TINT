...
function circleMask(cols: number, rows: number, cx: number, cy: number, r: number, ry: number = r): Mask {
...
function rectMask(cols: number, rows: number, x0: number, y0: number, x1: number, y1: number): Mask {
  const grid = emptyMask(cols, rows);
...
function wedgeMask(cols: number, rows: number, cx: number, maxHalf: number, pointAtBottom: boolean, y0 = 0, y1 = rows - 1): Mask {
  const grid = emptyMask(cols, rows);
  const span = y1 - y0 || 1;
  for (let y = y0; y <= y1; y++) {
    const t = (y - y0) / span;
...
function starMask(cols: number, rows: number, cx: number, cy: number, outerR: number, innerR: number, points: number): Mask {
  const grid = emptyMask(cols, rows);
...
function union(...masks: Mask[]): Mask {
...
function subtract(a: Mask, b: Mask): Mask {
...
function belowRow(cols: number, rows: number, y: number, keepBelow: boolean): Mask {
  const grid = emptyMask(cols, rows);
...
function outlineOf(mask: Mask, cols: number, rows: number): [number, number][] {
