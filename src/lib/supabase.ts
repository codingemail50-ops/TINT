...
// `||`, not `??` — an .env file with these keys present but left blank
// (e.g. copied from .env.example and never filled in) resolves to an empty
// string, not undefined, and `??` only falls back on null/undefined. An
// empty string would otherwise silently win, pointing the whole app at an
// empty Supabase URL instead of the real project below.
